...
if(measures.length > 0){
    measureLinks_display = measures.map(measure => {
        return (
            <Link id={measure._id} className="p-1" onClick={() => setCurrentMeasure(measure)}>
                {measure.title}
            </Link>
        )
    })
...
const getCircle = (number) => {
    switch(number) {
        case 0:
          return (<div className="rounded-circle bg-success" style={{height:'30px', width:'30px'}} ></div>)
        case 1:
          return (<div className="rounded-circle bg-warning" style={{height:'30px', width:'30px'}} ></div>)
        case 2:
            return (<div className="rounded-circle bg-danger" style={{height:'30px', width:'30px'}} ></div>)
        default: 
          return (<div className="rounded-circle " style={{height:'30px', width:'30px'}} ></div>)
      }
}

const getMaxFromThis = [currentMeasure.risk, currentMeasure.budget, currentMeasure.artefact]
const getMax = (array) => {
    let res = 0
    array.map(a => {
        if(a > res){res = a}
...
const getApprovedBudget = () => {
    ...
    if(artefacts.length > 0){
        artefacts.map(art => {
            if(art.budget){
                const parsed = parseInt((art.budget).substring(0, art.budget.length - 2))
...
const getKPIProgress = () => {
    ...
    return (
        ...
        <Col xl="6">
                    ...
                    <Card className="mini-stats-wid">
                        <CardBody >
                            ...
                            <Row className="mt-2 ">
                                ...
                                <Col >
                                     {getCircle(currentMeasure.budget)}
                                </Col> 
                            </Row>
                            <Row className="mt-2 ">
                                <Col >
                                ...
                                <Col >
                                    {getCircle(currentMeasure.artefact)}
                                </Col> 
                            </Row>
           ...
           <Row>
                <CardUser signal={ getMax(getMaxFromThis)} 
                            budget={ getApprovedBudget() }
                            numberOfMeasures={ currentMeasure.artefacts ? currentMeasure.artefacts.length : 0}
                            />
            </Row>  

            <Row>
                <TopCities overallProgress={getOverallProgress()} kpiProgress={getKPIProgress()} />
            </Row>  
